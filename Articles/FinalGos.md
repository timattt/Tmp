...
## Свойства и характеристики архитектуры ПО
...
Если мы хотим определить класс для точки из пространства признаков, то мы ищем k ближайших соседей и голосванием выбираем ответ.

### PCA (анализ главных компонент)

Хотим уменьшить размерность данных. Но при этом не потерять смысл.

![image](https://github.com/user-attachments/assets/7e12ec05-3dd0-4f1b-9f00-133d212e04d1)

## SVM

* SVM стремится найти оптимальную гиперплоскость, которая наилучшим образом разделяет данные разных классов в пространстве признаков.
* SVM выбирает гиперплоскость, которая максимизирует зазор (margin) между классами. Зазор – это расстояние между гиперплоскостью и ближайшими точками данных разных классов.
* Точки данных, которые лежат ближе всего к гиперплоскости и влияют на её положение, называются опорными векторами. Именно они определяют границу между классами.
* SVM может работать не только с линейно разделимыми данными, но и с нелинейными, используя ядровые функции. Ядровые функции позволяют проецировать данные в пространство более высокой размерности, где они могут быть линейно разделены.

## RandomForest

Множество деревьев: Случайный лес строит множество решающих деревьев (как правило, сотни или тысячи), каждое из которых обучается на случайном подмножестве данных и признаков.

Случайное подмножество данных: Для обучения каждого дерева случайным образом выбирается подмножество обучающих примеров (с возвращением, т.е. с возможностью повторения). Этот процесс называется бутстрап (bootstrap).

Случайное подмножество признаков: Для каждого узла каждого дерева случайным образом выбирается подмножество признаков, по которым происходит разделение данных.

Агрегирование результатов: Результаты работы всех деревьев агрегируются для получения окончательного прогноза. При классификации это может быть голосование (побеждает наиболее часто встречающийся класс), а при регрессии – среднее или медиана значений.

## Основы нейронных сетей

* персептрон - на вход вектор признаков, к нему применяется линейное преобразование. Значение интерпретируется как вероятность нахождения в таком-то классе. Для нахождения решаем задачу оптимизацию. Градиентным спуском.
* нейросеть - множество персептронов скрепленных вместе. Тоже решаем задачу оптимизации, но используем обратное распространение ошибки, чтобы подкручивать все веса.

## Подходы к обучению нейронных сетей

* Обучение с учителем: В обучении с учителем модель обучается на размеченных данных, где каждая входная точка связана с известным выходом или целевым значением.
* Обучение без учителя: Модель пытается обнаружить скрытые закономерности, структуры или зависимости в данных.
* Обучение с подкреплением: Модель (агент) обучается путем взаимодействия со средой, получая вознаграждение или штраф за свои действия.
* Полуавтоматическое обучение: Сочетание обучения с учителем и без учителя. Модель использует небольшое количество размеченных данных и большой объем неразмеченных данных для обучения.
